...
try:
    ...
    # define table
    DELTA_TABLE_PATH = 's3a://incremental-etl/fact_order_line'
    dt = (
        DeltaTable
        .createIfNotExists(spark)
        .addColumn("line_sk", dataType=LongType(), generatedAlwaysAs=IdentityGenerator())
        .addColumn("line_id", dataType=IntegerType())
        .addColumn("order_sk", dataType=IntegerType())
        .addColumn("partner_sk", dataType=IntegerType())    
        .addColumn("line_product_id", dataType=IntegerType()) # temporary
        .addColumn("line_name", dataType=CharType(100))    
        .addColumn("line_product_uom_qty", dataType=DoubleType())
        .addColumn("line_price_unit", dataType=DoubleType())
    ...
    incremental_params = build_incremental_parameters(
        spark,
        delta_input_path='s3a://incremental-etl/raw_sale_order_line',
    ...
    change_df = (
        ...
        .load('s3a://incremental-etl/raw_sale_order_line')
    )
    ...
    DIM_PARTNER_DELTA_PATH = "s3a://incremental-etl/dim_partner"
    dim_partner_df = (
        spark
        .read.format("delta")
        .load(DIM_PARTNER_DELTA_PATH)
    )
    ...
    transform_df = (
        ...
        .join(
            dim_order_df.select("order_id", "order_sk", "record_start_date", "record_end_date"), 
            [
                dim_order_df.order_id == change_df.order_id,
                change_df.event_ts >= dim_order_df.record_start_date,
                change_df.event_ts < dim_order_df.record_end_date
            ],
            "left"
        )
        .join(
            dim_partner_df.select("partner_id", "partner_sk", "record_start_date", "record_end_date"),
            [
                dim_partner_df.partner_id == change_df.partner_id,
                change_df.event_ts >= dim_partner_df.record_start_date,
                change_df.event_ts < dim_partner_df.record_end_date
            ],
            "left"
        )
    )
    ...
    if not transform_df.isEmpty():
        check = Check(spark, level=CheckLevel.Error, description="Quality check for fact_order_line")
        check_run = (
            ...
            .addCheck(
                ...
                .isUnique("id")
                .isComplete("order_sk")
                .isComplete("partner_sk")
                .isNonNegative("product_uom_qty")
                .isNonNegative("price_unit")
            )
            .run()
        )
    ...
    rows_to_insert = (
        ...
        .join(
            ...
            [
                existing_df.line_id == transform_df.id,
        ...
        .select(
            transform_df.id,
            transform_df.order_sk,
            transform_df.partner_sk,
            transform_df.product_id,
            transform_df.name,
            transform_df.product_uom_qty,
            transform_df.price_unit,
    ...
    rows_to_merge = (
        ...
        .withColumn("merge_key", F.col("id")) # update old records if matche. otherwise, insert
        .union(
            rows_to_insert
    ...
    (
        ...
        .merge(
            ...
            condition="source.merge_key = target.line_id"
        )
        .whenMatchedUpdate(
        ...
        .whenNotMatchedInsert(
            values={
                "target.line_id": "source.id",
                "target.order_sk": "source.order_sk",
                "target.partner_sk": "source.partner_sk",
                "target.line_product_id": "source.product_id",
                "target.line_name": "source.name",
                "target.line_product_uom_qty": "source.product_uom_qty",
                "target.line_price_unit": "source.price_unit",
