...
# define table
output_path = 's3a://incremental-etl/raw-sale-order'
dt = (
    DeltaTable
    .createIfNotExists(spark)
    .addColumn("id", dataType=IntegerType())
    .addColumn("partner_id", dataType=IntegerType())
    .addColumn("name", dataType=CharType(25))
    .addColumn("state", dataType=CharType(25))
    .addColumn("date_order", dataType=TimestampType())
    .addColumn("effective_date", dataType=TimestampType())
...
input_path = 's3a://incremental-etl/raw-sale-order-cdc/'
...
# hourly scheduler
# next_run = 2025-05-27 18:00:00 -> ts_ms in [17:00, 17:59]

# transform data
transform_df = (
    change_df
    .withColumn("id", coalesce(col("`after.id`"), col("`before.id`"))) 
    .withColumn("partner_id", coalesce(col("`after.partner_id`"), col("`before.partner_id`"))) 
    .withColumn("name", coalesce(col("`after.name`"), col("`before.name`"))) 
    .withColumn("state", coalesce(col("`after.state`"), col("`before.state`")))
    # micro timestamps
    .withColumn("date_order", from_unixtime(coalesce(col("`after.date_order`"), col("`before.date_order`")) / lit(1_000_000)).cast(TimestampType()))
    .withColumn("effective_date", from_unixtime(coalesce(col("`after.effective_date`"), col("`before.effective_date`")) / lit(1_000_000)).cast(TimestampType()))
