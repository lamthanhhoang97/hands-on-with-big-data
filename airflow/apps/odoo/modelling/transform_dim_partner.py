...
try:
    ...
    # define table
    DELTA_TABLE_PATH = 's3a://incremental-etl/dim_partner'
    dt = (
        DeltaTable
        .createIfNotExists(spark)
        .addColumn("partner_sk", dataType=LongType(), generatedAlwaysAs=IdentityGenerator())
        .addColumn("partner_id", dataType=IntegerType())
        .addColumn("partner_name", dataType=CharType(50))    
        .addColumn("partner_email", dataType=CharType(50))        
        .addColumn("partner_phone", dataType=CharType(50))
        .addColumn("partner_street", dataType=CharType(50))
        .addColumn("partner_city", dataType=CharType(50))
    ...
    incremental_params = build_incremental_parameters(
        spark,
        delta_input_path='s3a://incremental-etl/raw_res_partner',
    ...
    change_df = (
        ...
        .load('s3a://incremental-etl/raw_res_partner')
    )
    ...
    if not transform_df.isEmpty():
        # quality checks
        check = Check(spark, level=CheckLevel.Error, description="Quality check for dim_parter")
        check_run = (
    ...
    rows_to_insert = (
        ...
        .join(
            ...
            [
                existing_df.partner_id == transform_df.id,
    ...
    (
        ...
        .merge(
            ...
            condition="source.merge_key = target.partner_id"
        )
        .whenMatchedUpdate(
        ...
        .whenNotMatchedInsert(
            values={
                "target.partner_id": "source.id",
                "target.partner_name": "source.name",
                "target.partner_email": "source.email",
                "target.partner_phone": "source.phone",
                "target.partner_street": "source.street",
                "target.partner_city": "source.city",
