...
def main():
    ...
    # read data
    hdfs_path = f"hdfs://hadoop:9000/user/hadoopuser/stage/stg_customer_dim"
    df = spark.read.format("delta").load(hdfs_path)

    # data quality checks
    table_name = "stg_customer_dim"
    check = Check(spark, CheckLevel.Error, f"Quality Check for table: {table_name}")
    checkResult = (
        ...
        .addCheck(
            ...
            .isUnique("sk_custkey")
            .isComplete("c_custkey")            
            .isComplete("record_active_flag")
            .isComplete("record_start_date")
        )
        .run()
    )
